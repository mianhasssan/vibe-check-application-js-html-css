...
// const btnStart = document.getElementById("start-btn");    //! Using event in JavaScript
...
function showQuestion()
{
    ...
    qCounterEl.textContent = `Question ${currentIndex + 1} of ${question.length}`;
    ...
    for (let i = 0; i < question.choices.length;i++){
        ...
        btn.innerHTML = `<span class="choice-tag">${labels[i]}</span> ${choices.label}`;

        btn.addEventListener("click",function(){
            selectChoice(btn,choices.points);
        });
